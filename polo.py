...
st.set_page_config(
    page_title="Volkswagen Polo VI 2018 - leboncoin",
    page_icon="🧡",
    layout="wide",
...
img1_b64 = get_base64_img("photo1.jpg")
img2_b64 = get_base64_img("photo2.jpg")
img3_b64 = get_base64_img("photo3.jpg")
...
<style>
    ...
    .vendeur-avatar {
        ...
        background-color: #2D3100; display: inline-block;
    }
    .btn-reserver {
...
# --- FIL D'ARIANE ---
st.markdown('<div class="breadcrumb">Accueil > Voitures > Nord-Pas-de-Calais > Nord > Mouvaux 59420 > Volkswagen Polo VI 2018</div>', unsafe_allow_html=True)
...
with col_left:
    ...
    st.markdown(f"""
        <div class="gallery-wrapper">
            <div class="main-photo-box">
                <img src="{img1_b64}" alt="Photo 1">
            </div>
            <div class="side-photos-box">
                <div class="side-photo-item"><img src="{img2_b64}" alt="Photo 2"></div>
                <div class="side-photo-item"><img src="{img3_b64}" alt="Photo 3"></div>
            </div>
        </div>
        <div class="price-box">
            <div class="ad-title">Volkswagen Polo VI 2018</div>
            <div class="ad-sub">Mouvaux · 2018 · 80000 km · Essence</div>
            <div class="ad-price">12 000 €</div>
            <div style="font-size: 13px; text-decoration: underline; font-weight: 600; margin-bottom: 6px;">📑 Simuler mon financement</div>
            <div style="font-size: 11px; color: #9CA3AF;">aujourd'hui à 16:13</div>
    ...
    st.markdown("""
        <div style="display: flex; gap: 30px; margin-top: 20px; font-size: 13px; font-weight: 600; color: #003B63;">
            ...
            <div>🏠 Votre contrat dès aujourd'hui</div>
        </div>
    ...
    st.markdown("""
        <div style="display: flex; gap: 30px; margin-top: 15px;">
            <div class="plus-item">
                ...
                <div class="plus-text">Annonce<br>récente</div>
            </div>
            <div class="plus-item">
                <div class="plus-icon-box">🚗</div>
    ...
    st.markdown("""
        ...
        <div class="serenite-list">
            🔒 <b>Réservation du véhicule</b><br>
            💳 <b>Paiement sécurisé</b><br>
            🛠️ <b>Garantie Panne Mécanique** dès 139 €</b>
        </div>
        <div style="font-size: 13px; font-weight: 700; text-decoration: underline; margin-top: 10px;">En savoir plus</div>
        <hr class="section-divider">
    """, unsafe_allow_html=True)
    ...
    infos_base = [
        ("📋 Marque", "VOLKSWAGEN", "📋 Modèle", "Polo"),
        ("📅 Année modèle", "2018", "🏎️ Kilométrage", "80000 km"),
        ("⛽ Énergie", "Essence", "🕹️ Boîte de vitesse", "Manuelle"),
        ("🚗 Nombre de portes", "5", "👥 Nombre de place(s)", "5")
    ]

    infos_extra = [
        ("🗓️ Date CT", "2028", "🚦 Mise en circulation", "01/2018"),
        ("✨ État du véhicule", "Excellent état", "🚙 Type de véhicule", "Berline"),
        ("💺 Sellerie", "Tissu", "🎨 Couleur", "Gris"),
        ("🌱 Crit'Air", "1", "⚡ Puissance fiscale", "4 Cv"),
        ("🐎 Puissance DIN", "65 Ch", "🪪 Permis", "Avec permis")
    ]

    if "show_all" not in st.session_state:
        st.session_state.show_all = False

    display_infos = infos_base + (infos_extra if st.session_state.show_all else [])
    ...
    if not st.session_state.show_all:
        if st.button("Voir les 10 critères supplémentaires"):
            st.session_state.show_all = True
            st.rerun()
    else:
        if st.button("Masquer les critères"):
            st.session_state.show_all = False
            st.rerun()
    ...
    st.write("Volkswagen Polo VI 2018 — Excellente affaire proposée proche de la cote avec seulement 80 000 km. Contrôle technique OK.")
    st.write("Entretien propre et rigoureux : factures Volkswagen à l'appui et carnet d'entretien tamponné.")
    st.write("Parfait état général, idéal pour rouler sereinement. Prix à débattre, premier contact par message.")
    ...
    st.text_input("Montant du financement", value="12 000 €", key="montant_financement")
    ...
    st.markdown("<div style='font-size:15px; font-weight:700; color:#111827; margin-bottom:12px;'>Mouvaux (59420)</div>", unsafe_allow_html=True)

    st.markdown("""
        <iframe width="100%" height="320" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" 
        src="https://www.openstreetmap.org/export/embed.html?bbox=3.1100%2C50.6850%2C3.1700%2C50.7200&amp;layer=mapnik&amp;marker=50.7028%2C3.1408" 
    ...
    with col_v1:
        st.markdown("""
            <div style="display: flex; align-items: center; gap: 14px;">
                <div class="vendeur-avatar" style="width: 56px; height: 56px;"></div>
                <div>
                    <div style="font-weight: 900; font-size: 18px; color: #111827;">mhventes</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
    with col_v2:
    ...
    st.markdown("""
        <div style="font-size: 13px; color: #4B5563; margin-top: 12px; display: flex; flex-direction: column; gap: 4px;">
            <div>📅 Membre depuis juin 2021</div>
            <div>⏱️ Dernière activité il y a 11 minutes</div>
        </div>

        <div class="vendeur-badges-container">
            <div class="vendeur-badge-item">
                <div class="vendeur-badge-circle">💬</div>
                <div>Très réactif</div>
            </div>
            <div class="vendeur-badge-item">
                <div class="vendeur-badge-circle">📱</div>
    ...
    st.markdown("""
        <div class="vendeur-card">
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 10px;">
                <div class="vendeur-avatar"></div>
                <div style="font-weight: 800; font-size: 16px;">mhventes</div>
            </div>
            <div style="background: #FFF7ED; color: #C2410C; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 10px; display: inline-block;">💬 Très réactif</div>
            <div style="font-size: 12px; color: #6B7280; margin: 8px 0 15px 0;">⏱️ Dernière activité il y a 11 minutes</div>
